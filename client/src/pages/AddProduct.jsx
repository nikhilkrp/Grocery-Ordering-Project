    ...
    return (
        <div className=" no-scrollbar flex-1 h-[95vh] overflow-y-scroll flex flex-col justify-between">
            <form onSubmit={onSubmitHandler} className="md:p-10 p-4 space-y-5 max-w-lg">
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-2">
                        {Array(4).fill('').map((_, index) => (
                            <label key={index} htmlFor={`image${index}`}>
                                ...
                                <img className='max-w-24 cursor-pointer' src={files[index]?URL.createObjectURL(files[index]): assets.upload_area} alt='upload_area' width={100} height={100}/>
                            </label>
                        ))}
                    </div>
                </div>    
                ...
                <button className= " cursor-pointer px-8 py-2.5 bg-green-600 text-white font-medium rounded">ADD</button>
            </form>
        </div>
    );
};
